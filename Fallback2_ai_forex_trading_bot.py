...
class AITradingStrategy:
    ...
    def generate_signal(self, data_dict, risk_percent=1.0, tp_multiplier=2.0, sl_multiplier=1.0):
        ...
        try:
            ...
            # Check trading hours (8am-5pm UTC)
            current_hour = datetime.now().hour
            if not (8 <= current_hour < 17):
                signal.update({
                    "signal": "HOLD",
                    "reason": "Outside active trading hours",
                    "confidence": 0.0
                })
                return signal
...
class TradingBotGUI(QMainWindow):
    ...
    def create_account_tab(self):
        ...
        self.refresh_account_btn.clicked.connect(self.update_account_display)
        self.account_layout.addWidget(self.refresh_account_btn)
    ...
    def start_bot(self):
        ...
        try:
            ...
            self.log_message(f"Error starting bot: {str(e)}", level="error")
    ...
    def stop_bot(self):
        ...
        try:
            ...
            self.log_message(f"Error stopping bot: {str(e)}", level="error")
    ...
    def handle_new_data(self, data_dict):
        ...
        try:
            ...
            self.log_message(f"Error processing new data: {str(e)}", level="error")
    ...
    def save_model(self):
        ...
        if file_path:
            try:
                ...
                self.log_message(f"Error saving model: {str(e)}", level="error")
    ...
    def load_model(self):
        ...
        if file_path:
            try:
                ...
                self.log_message(f"Error loading model: {str(e)}", level="error")
    ...
    def execute_trade(self, signal):
        ...
        try:
            ...
            self.log_message(f"Trade execution failed: {str(e)}", level="error")
    ...
    def create_order(self, side, price, units, take_profit, stop_loss):
        ...
        try:
            ...
            self.log_message(f"Error creating {side} order: {str(e)}", level="error")
            return False
    ...
    def close_position(self):
        ...
        try:
            ...
            # Get open trades
            endpoint = TradesList(accountID=account_id)
            response = self.api.request(endpoint)
            ...
            self.log_message(f"Error closing position: {str(e)}", level="error")
            return False
            
    def get_account_info(self):
        """Get account information from OANDA with detailed error handling"""
        try:
            account_id = self.account_id_input.text().strip()
            if not account_id:
                self.log_message("Account ID not set", level="error")
                return None
            ...
            if not api_key:
                self.log_message("API Key not set in settings", level="error")
                return None
                
            # Reinitialize API with current credentials
            self.api = API(access_token=api_key, environment=environment)
            ...
            try:
                response = self.api.request(endpoint)
                self.log_message(f"Raw API response: {response}", level="debug")
                
                if 'account' not in response:
                    error_msg = response.get('errorMessage', 'No account data in response')
                    self.log_message(f"API Error: {error_msg}", level="error")
                    return None
                
            # Log successful connection
                self.log_message("Successfully fetched account info", level="info")
                ...
                return {
                    ...
                    'leverage': int(float(response['account']['marginRate']))
                }
       
            except Exception as e:
                self.log_message(f"API Request Failed: {str(e)}", level="error")
                return None
            
        except Exception as e:
            self.log_message(f"Error getting account info: {str(e)}", level="error")
            return None
            
    def update_account_display(self):
        """Update the account information display with better error handling"""
        try:
            account_info = self.get_account_info()
            
            if not account_info:
                self.balance_label.setText("Failed to fetch")
                self.equity_label.setText("Check logs")
                self.log_message("Account info fetch failed - verify API key/account ID", "error")
                return
            ...
            self.log_message(f"Error updating account display: {str(e)}", level="error")
    ...
    def update_positions_table(self):
        ...
        try:
            ...
            for trade in response['trades']:
                ...
                self.positions_table.setItem(row, 3, QTableWidgetItem(trade['currentUnits']))
            ...
            self.log_message(f"Error updating positions table: {str(e)}", level="error")
